...
test('emitirRelatorio: Serpro 500 mapeia para HttpError 503 e errors.code MEI_GUIDE_SERPRO_UNAVAILABLE', async () => {
  __setHttpClientsForTests({
    noMtls: true,
    fetchFn: async (url) => {
      const u = String(url);
  ...
  const { emitirRelatorio } = await import('../src/services/gestao/emitir.service.js');

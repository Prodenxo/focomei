/**
 * Validação de dígitos verificadores CPF/CNPJ (BR), alinhada ao uso típico do backend fiscal.
 * Apenas strings de dígitos (sem máscara).
 */

const allSameDigits = (d: string) => /^(\d)\1+$/.test(d);

/** CPF: 11 dígitos, não sequência trivial, DV válidos. */
...
/** CNPJ: 14 dígitos, não sequência trivial, DV válidos. */

/** @typedef {'corrente'|'poupanca'|'cartao_credito'|'dinheiro'|'outro'} ContaFinanceiraTipo */

/**
 * @typedef {Object} BankCatalogEntry
 * @property {string} id
 * @property {string} nome
 * @property {string} [libraryNome]
 * @property {string} cor
 * @property {string[]} [keywords]
 * @property {ContaFinanceiraTipo} [defaultTipo]
 */
...
export function resolveLibraryNome(entry) {
...
export function resolveContaAccent(conta) {
  const bank = findBankById(conta?.instituicao_id) ?? findBankByNome(conta?.nome);
  return conta?.cor || bank?.cor || '#64748B';
}
